...
class Ensemble:
    def __init__(
        self, log_rules, save_path, starting_cash, agent_classes, args: Config
    ):
    ...
    def multi_trade(self):
        ...
        states = torch.zeros(
            (trade_env.max_step, self.num_envs, self.state_dim), dtype=torch.float32
        ).to(self.device)
        multi_actions = torch.zeros(
            (trade_env.max_step, self.num_envs, 1), dtype=torch.int32
        ).to(self.device)  # different
        rewards = torch.zeros(
            (trade_env.max_step, self.num_envs), dtype=torch.float32
        ).to(self.device)
        dones = torch.zeros((trade_env.max_step, self.num_envs), dtype=torch.bool).to(
            self.device
        )
        ...
        for i in range(trade_env.max_step):
            ...
            for ai, agent in enumerate(agents):
                ...
                tensor_state = torch.as_tensor(
                    intermediate_state, dtype=torch.float32, device=agent.device
                )
